...
CREATE TABLE `ams_active`  (
  ...
  `is_enable` tinyint(1) NULL DEFAULT NULL COMMENT '是否启用:0->否,1->是',
  `integration` int(5) NULL DEFAULT NULL COMMENT '获得的积分',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '活动信息表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `ams_active_comment`  (
  ...
  `is_show` tinyint(1) NULL DEFAULT 1 COMMENT '是否显示在手机端:0->否,1->是',
...
CREATE TABLE `oms_order`  (
  ...
  `pay_type` tinyint(1) NULL DEFAULT 0 COMMENT '支付方式:0->未支付,1->微信支付,2->支付宝支付',
  `source_type` tinyint(1) NULL DEFAULT 0 COMMENT '订单来源:0->app订单,1->PC订单',
  `order_status` tinyint(1) NULL DEFAULT 0 COMMENT '订单状态：0->待付款；1->待发货；2->已发货；3->已完成；4->已关闭；5->无效订单',
  `order_type` tinyint(1) NULL DEFAULT 0 COMMENT '订单类型：0->正常订单；1->秒杀订单',
...
CREATE TABLE `oms_product_comment`  (
  ...
  `nickname` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '用户昵称',
  `comment_content` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '评价内容',
  `comment_star` tinyint(1) NULL DEFAULT NULL COMMENT '评价星级: 1-5颗星',
  `is_show` tinyint(1) NULL DEFAULT 1 COMMENT '是否显示在手机端:0->否,1->是',
  `create_time` datetime(0) NULL DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `product_id_index`(`product_id`) USING BTREE,
...
CREATE TABLE `pms_product`  (
  ...
  `hot_status` tinyint(1) NULL DEFAULT 0 COMMENT '热销状态: 0->否;1->是',
  `exchange_status` tinyint(1) NULL DEFAULT 0 COMMENT '兑换状态:0->否;1->是',
...
CREATE TABLE `pms_product_attribute`  (
  ...
  `attr_type` tinyint(1) NULL DEFAULT NULL COMMENT '属性的类型；0->规格；1->参数',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '商品属性参数表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `pms_product_attribute_category`  (
  ...
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '商品属性分类表' ROW_FORMAT = Dynamic;
...
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '商品属性参数信息表' ROW_FORMAT = Dynamic;
...
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '商品分类目录表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `pms_sku_stock`  (
  ...
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `product_id_index`(`product_id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = 'sku库存表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `ums_user`  (
  ...
  `act_status` tinyint(1) NULL DEFAULT NULL COMMENT '账号启用状态：0->禁用,1->启用',
  `create_time` datetime(0) NOT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE
...
CREATE TABLE `ums_user_member_level`  (
  ...
  `default_status` tinyint(1) NULL DEFAULT NULL COMMENT '是否为默认等级:0->否,1->是',
  `priviledge_birthday` tinyint(1) NULL DEFAULT NULL COMMENT '是否有生日特权:0->否,1->是',
  PRIMARY KEY (`id`) USING BTREE
...
CREATE TABLE `ums_user_receive_address`  (
  ...
  `default_status` tinyint(1) NULL DEFAULT NULL COMMENT '默认地址：0->否，1->是',
  PRIMARY KEY (`id`) USING BTREE
...
CREATE TABLE `ums_user_recharge_record`  (
  ...
  `change_type` tinyint(1) NOT NULL COMMENT '金额变动类型:0->消费,1->充值',
  `recharge_status` tinyint(1) NULL DEFAULT NULL COMMENT '充值状态:0->失败，1->成功',
